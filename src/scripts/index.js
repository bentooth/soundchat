...
if (mySongsComponent) {
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      readSongsFromFirestore(user.uid).then((songs) => {
        songs.forEach((song) => {
...
if (audioElement && artistNameElement && songSelectElement && commentsContainer) {
  ...
  // Set artist name
  getArtistName(userId).then((artistName) => {
    artistNameElement.innerText = artistName;
  });

  // Set all songs for the user id
  readSongsFromFirestore(userId).then((songs) => {
    songs.forEach((song) => {
